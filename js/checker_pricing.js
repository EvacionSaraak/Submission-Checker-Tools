    ...
    async function handleRun() {
      ...
      try {
        ...
        const output = extracted.map(rec => {
          ...
          // This initial value may be overridden only when the clinician is actually Endodontics.
          let pricingContext;
          ...
          if (xmlNet === 0) {
            ...
            if (xmlQty <= 0) {
              remarks.push(xmlQty === 0 ? 'Quantity is 0 (invalid)' : 'Quantity is less than 0 (invalid)');
            }
            ...
            if (endoEntry && refPrice === null) {
              remarks.push(`Code ${rec.CPT} has no available price under ${pricingContext}.`);
            }
      ...
      for (const r of rows) {
        ...
        html += `
          <tr class="${cls}">
            ...
            <td>${r.PricingRow ? `<button type="button" class="btn btn-sm btn-outline-primary" onclick="window.showPricingComparison(${r._originalIndex})">View</button>` : ''}</td>
          </tr>
        `;
    ...
    function showComparisonModal(index) {
      ...
      const modalHtml = `
        <div class="modal-content">
          <button type="button" class="modal-close" onclick="window.closePricingComparison()">×</button>
          <h3>Price Comparison</h3>
          <div class="row">
            <div class="col">${xmlTable}</div>
            <div class="col">${pricingTable}</div>
          </div>
          <button type="button" class="btn btn-secondary" onclick="window.closePricingComparison()">Close</button>
        </div>
      `;
