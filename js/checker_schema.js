(function() {
  try {
    ...
    function exportErrorsToXLSX(data, schemaType) {
      ...
      const errorRows = rows.filter(row => row.Remark !== "OK");
        ...
        fetch('../json/clinician_licenses.json')
          ...
          .then(rows => {
            ...
            (Array.isArray(rows) ? rows : []).forEach(row => {
              ...
              // Do not overwrite a valid specialty with an empty one.
    ...
    function getSelectedClaimTypeMode() {
      ...
      if (claimTypeMedical && claimTypeMedical.checked) {
        return 'MEDICAL';
      }

      if (claimTypeDental && claimTypeDental.checked) {
        return 'DENTAL';
      }
    ...
    function collectNotMergedClaimContext(
      ...
      const memberID =
        safeTextByTag(claim, 'MemberID')
          .toUpperCase();

      const payerID =
        safeTextByTag(claim, 'PayerID')
          .toUpperCase();

      const providerID =
        safeTextByTag(claim, 'ProviderID')
          .toUpperCase();
      ...
      const facilityID =
        safeTextByTag(encounter, 'FacilityID')
          .toUpperCase();

      const encounterStartRaw =
        safeTextByTag(encounter, 'Start');

      const encounterEndRaw =
        safeTextByTag(encounter, 'End');
    ...
    function validateConsultationAndSpecialtyRules(
      ...
      activityContexts.forEach(ctx => {
        ...
        if (
          MUTUALLY_EXCLUSIVE_INFUSION_CODES
            .has(ctx.code)
        ) {
        ...
        // 82948 is exempt from the Pathology specialty requirement.
        if (
          /^8/.test(ctx.code) &&
          ctx.code !== '82948' &&
          !specialtyContains(
    ...
    function validateClaimSchema(
      ...
      for (const claim of claims) {
        ...
        if (
          claimID &&
          notMergedRemarksByClaim.has(
            claimID
          )
        ) {
          const notMergedRemarks =
            notMergedRemarksByClaim.get(
              claimID
            ) || [];

          notMergedRemarks.forEach(
            remark => {
