...
self.onmessage = async e => {
  ...
  try {
    ...
    // Transfer the underlying ArrayBuffer properly
...
async function combineReportings(fileEntries, fileTypes) {
  ...
  for (let i = 0; i < fileEntries.length; i++) {
    ...
    // Detect Facility ID from Visit Id if available
    let facilityID = '';
    ...
    for (let r = 1; r < sheetData.length; r++) {
      ...
      for (const tgtHeader of TARGET_HEADERS) {
        ...
        if (tgtHeader === 'Pri. Patient Insurance Card No') {
          // ClinicPro can have PatientCardID or Member ID
          let val = '';
          if (isClinicPro) {
      ...
      // Fix Encounter Date to formatted DD/MM/YYYY if possible
      const encDateIdx = TARGET_HEADERS.indexOf('Encounter Date');
      if (encDateIdx >= 0) {
        const rawDate = targetRow[encDateIdx];
        const parsedDate = DateHandler.parse(rawDate);
        targetRow[encDateIdx] = parsedDate ? DateHandler.format(parsedDate) : rawDate;
      }
