(function () {
  ...
  function handleXmlInput() {
    ...
    file.text().then(text => {
      ...
      updateResultsDiv(); toggleProcessButton();
    }).catch(e => {
    ...
    setTimeout(() => {
      ...
      claimNodes.forEach(cl => {
        ...
        activities.forEach(act => {
          ...
          const od = map[oid] || defaultClinicianData(), pd = map[pid] || defaultClinicianData();
          const rowRemarks = [];
          ...
          if (clinicianStatusMap) {
            ...
            if (oid && encounterDate) {
              ...
              if (ordRec && ordRec.status && ordRec.status.toLowerCase() === 'inactive') rowRemarks.push(`Ordering Clinician (${oid}) has INACTIVE license as of ${ordRec.effectiveDate}`);
            }
          ...
          results.push({
            claimId: cid, activityId: aid, activityStart: encounterStartStr,
            orderingId: oid, orderingName: od.name, orderingCategory: od.category, orderingPrivileges: od.privileges || '', orderingFrom: od.from || '', orderingTo: od.to || '', orderingEligibility: ordXlsxRow ? ordXlsxRow.eligibility : 'N/A',
            performingId: pid, performingName: pd.name, performingCategory: pd.category, performingPrivileges: pd.privileges || '', performingFrom: pd.from || '', performingTo: pd.to || '', performingEligibility: perfXlsxRow ? perfXlsxRow.eligibility : 'N/A',
            status: '', packageName: '', serviceCategory: '', consultationStatus: '', effectiveDate: '', expiryDate: '', cardNumber: '', cardStatus: '',
            valid: rowRemarks.length === 0, remarks: rowRemarks
          });
        });
      });
  ...
  function validateClinicianStatus(clinicianId, providerId, encounterStartStr) {
    const remarks = [], records = clinicianStatusMap[clinicianId];
    let eligible = true;
  ...
  function checkEligibility(encounterStartStr, encounterEndStr, xlsxRow) {
    const encounterStart = parseDate(encounterStartStr), encounterEnd = parseDate(encounterEndStr);
    const effectiveDate = new Date(xlsxRow.effectiveDate || xlsxRow.from), expiryDate = new Date(xlsxRow.expiryDate || xlsxRow.to);
    const remarks = [];
    let eligible = true;
    if (isNaN(encounterStart) || isNaN(encounterEnd)) { remarks.push("Invalid Encounter dates in XML"); eligible = false; }
    else if (!effectiveDate || !expiryDate || isNaN(effectiveDate) || isNaN(expiryDate)) { remarks.push("Invalid Effective/Expiry dates in Excel"); eligible = false; }
    else if (!(encounterStart >= effectiveDate && encounterEnd <= expiryDate)) { remarks.push("Procedure is done outside of Eligibility window"); eligible = false; }
  ...
  function renderSummary(results) {
    const validCount = results.filter(r => r.valid).length, total = results.length, pct = total ? Math.round((validCount / total) * 100) : 0;
  ...
  function renderTableHeader() {
    ...
    ['Claim ID','Activity ID','Activity Start (Encounter Date)','Ordering Clinician','Performing Clinician','License Status','Eligibility','Valid','Remarks']
      .forEach(text => { const th = document.createElement('th'); th.scope = 'col'; th.textContent = text; row.appendChild(th); });
  ...
  function parseDate(dateStr) {
    if (!dateStr) return new Date('Invalid');
    if (!isNaN(dateStr)) { const excelSerial = parseFloat(dateStr); if (!isNaN(excelSerial)) return new Date((excelSerial - (25567 + 2)) * 86400 * 1000); }
    let d = new Date(dateStr); if (!isNaN(d.getTime())) return d;
    d = new Date(dateStr.replace(/(\d+)\/(\d+)\/(\d+)/, '$2/$1/$3')); if (!isNaN(d.getTime())) return d;
    d = new Date(dateStr.replace(/(\d+)-(\d+)-(\d+)/, '$1/$2/$3')); if (!isNaN(d.getTime())) return d;
  ...
  function appendCell(tr, content, { isHTML = false, isArray = false, verticalAlign = '' } = {}) {
    ...
    if (isArray) {
      td.style.whiteSpace = 'pre-line'; td.textContent = '';
      (Array.isArray(content) ? content : [content]).forEach(text => {
        const div = document.createElement('div'); div.textContent = text; td.appendChild(div);
      });
    } else if (isHTML) { td.style.whiteSpace = 'pre-line'; td.innerHTML = content; }
    else { td.style.whiteSpace = String(content).includes('\n') ? 'pre-line' : 'nowrap'; td.textContent = content; }
    tr.appendChild(td);
  }
