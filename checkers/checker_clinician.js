...
var xmlInput = document.getElementById('xmlFileInput');
var excelInput = document.getElementById('excelFileInput');
var openJetInput = document.getElementById('openJetFileInput');
...
function initEventListeners() {
  ...
  processBtn.addEventListener('click', function() {
    if (xmlDoc && clinicianMap && openJetClinicianList.length > 0) processClaims(xmlDoc, clinicianMap);
  });
}
...
function handleUnifiedExcelInput() {
  ...
  if (excelInput.files[0]) {
    promises.push(sheetToJsonWithHeader(excelInput.files[0], 0, 2).then(function(data) {
      clinicianMap = {};
  ...
  if (openJetInput.files[0]) {
    promises.push(sheetToJsonWithHeader(openJetInput.files[0], 0, 1).then(function(data) {
  ...
  if (openJetInput.files[0]) {
    promises.push(sheetToJsonWithHeader(openJetInput.files[0], 0, 1).then(function(data) {
      ...
      if (data.length) {
        ...
        var licKey = keys.find(function(k){ return /license$/i.test(k); }) || keys[0];
        data.forEach(function(row) {
          var lic = (row[licKey] || '').toString().trim();
          if (lic) openJetClinicianList.push(lic);
        });
      }
    }));
...
function getText(p, tag) { var el = p.getElementsByTagName(tag)[0]; return el ? el.textContent.trim() : ''; }
function defaultClinicianData() { return {name:'Unknown',category:'Unknown',privileges:'Unknown'}; }
function validateClinicians(o, p, od, pd) { if (!o || !p) return false; if (o === p) return true; return od.category === pd.category; }
function generateRemarks(od, pd) { var r = []; if (od.category !== pd.category) r.push('Category mismatch (' + od.category + ' vs ' + pd.category + ')'); return r.join('; '); }
...
function processClaims(d, m) {
  ...
  claims.forEach(function(cl) {
    ...
    acts.forEach(function(act) {
      ...
      var rem = [];
      if (pid && openJetClinicianList.indexOf(pid) === -1) rem.push('Performing Clinician (' + pid + ') not in Open Jet');
      if (oid && openJetClinicianList.indexOf(oid) === -1) rem.push('Ordering Clinician (' + oid + ') not in Open Jet');
      var valid = validateClinicians(oid, pid, od, pd);
      if (!valid) rem.push(generateRemarks(od, pd));
      res.push({
        claimId: cid,
        activityId: aid,
...
function renderResults(results) {
  ...
  var vc = results.filter(function(r) { return r.valid; }).length;
  ...
  results.forEach(function(r) {
    ...
    var tr = document.createElement('tr'); tr.className = r.valid ? 'valid' : 'invalid';
    var td0 = document.createElement('td'); td0.textContent = r.claimId;
    if (r.rowSpan > 1) { td0.rowSpan = r.rowSpan; td0.style.verticalAlign = 'top'; }
    tr.appendChild(td0);
...
function setupExportHandler(results) {
  ...
  exportCsvBtn.onclick = function() {
    ...
    var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'validation_results.csv'; a.click();
  };
}
